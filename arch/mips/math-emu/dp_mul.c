...
ieee754dp ieee754dp_mul(ieee754dp x, ieee754dp y)
{
    ...
    case CLPAIR(IEEE754_CLASS_SNAN,IEEE754_CLASS_SNAN):
      return ieee754dp_nanxcpt(ieee754dp_bestnan(x,y),"mul",x,y);
...
#define DPXMULT(x,y)	((unsigned long long)(x) * (unsigned long long)y)

	{
		...
		{
		    unsigned long long at = lrm + (t << 32);
		    hrm += at < lrm;
		    lrm = at;
		}
		hrm = hrm + (t >> 32);
	...
	if((signed long long)rm < 0) {
	    rm = (rm >> (64-(DP_MBITS+1+3))) | ((rm << (DP_MBITS+1+3)) != 0);
	    re++;
	} else {
	    rm = (rm >> (64-(DP_MBITS+1+3+1))) | ((rm << (DP_MBITS+1+3+1)) != 0);
	}
